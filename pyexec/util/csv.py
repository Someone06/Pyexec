...
class CSV:
    ...
    def to_stats(self, info: PackageInfo) -> PyexecStats:
        name = info.name
        ...
        github_repo_created_at = (
            datetime.min if info.github_info is None else info.github_info.created_at
        )
        github_repo_last_updated = (
            datetime.min if info.github_info is None else info.github_info.last_updated
        )
        ...
        has_setuppy = False if info.repo_info is None else info.repo_info.has_setuppy
        has_requirementstxt = (
            False if info.repo_info is None else info.repo_info.has_requirementstxt
        )
        has_makefile = False if info.repo_info is None else info.repo_info.has_makefile
        has_pipfile = False if info.repo_info is None else info.repo_info.has_pipfile
        loc = (
            -1
        ...
        pip_dependency_count = (
            -1 if info.dockerfile is None else info.dockerfile.pip_dependency_count()
        )
        apt_dependency_count = (
            -1 if info.dockerfile is None else info.dockerfile.apt_dependency_count()
        )
        ...
        percentage_covered = (
            -1 if info.test_result is None else info.test_result[1].percentage_covered
        )
        missing_lines = (
